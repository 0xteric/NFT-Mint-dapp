...
export default function ListedNFTS({ listings, sortBy, sortDir, refetchTotalSales, refetchTotalVolume }: ListedNFTSProps) {
  ...
  const handleBuy = async (tokenId: bigint, price: bigint) => {
    try {
      setTx(tokenId, { txStatus: "waiting", action: "buy" })
      ...
      setTimeout(() => {
        setTx(tokenId, { txStatus: "idle", action: "none", txHash: undefined })
        queryClient.invalidateQueries({ queryKey: ["marketplace-index"] })
        refetchTotalSales()
        refetchTotalVolume()
      }, 3500)
    } catch (e: any) {
  ...
  const handleBidToken = async (tokenId: bigint, price: number) => {
    try {
      setTx(tokenId, { txStatus: "waiting", action: "bidToken" })
      const _price = parseEther(String(price))

      const hash = await bidToken(tokenId, _price)
        ...
        {sortedListings.map((nft, idx) => {
          ...
          return (
            <AnimatePresence key={nft.id}>
              <motion.div id="buy-btn" initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} transition={{ duration: 0.3, delay: idx * 0.1 }}>
                <div
                  id="buy-btn"
                  onClick={() => {
                ...
                >
                  ...
                  {address ? (
                    <div
                      id="buy-btn"
                      className={
                        "absolute flex items-center w-[90%] justify-center gap-2 transition-all duration-300 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 " +
                        (Number(offerId) != Number(nft.tokenId) ? " scale-0 opacity-15" : " scale-100 opacity-100")
                      }
                    >
                      {Number(offerId) == Number(nft.tokenId) && (
                        <div id="buy-btn" className="border border-(--accent)/70 card rounded justify-center items-center  flex flex-col  w-full z-200">
                          {(tx.txStatus && (
                            <div className="flex flex-col w-full">
                              {(tx.txStatus == "idle" || tx.action != "buy") && !bidForm && (
                                <button
                                  id="buy-btn"
                                  onClick={() => handleBuy(nft.tokenId, nft.price)}
                                  className=" bg-transparent! w-full p-4  text-(--accent)! hover:opacity-70! transition-opacity duration-200 z-200"
                                >
                                  Buy
                                </button>
                              )}
                              {tx.txStatus == "waiting" && tx.action == "buy" && (
                                <button id="buy-btn" className=" bg-transparent! w-full p-4  text-(--accent)! hover:opacity-70! transition-opacity duration-200 z-200">
                                  Sign...
                                </button>
                              )}
                              {(tx.txStatus == "idle" || tx.action != "bidToken") && bidForm && (
                                <div id="buy-btn" className="flex  items-center  gap-2 bg-transparent! w-full  text-(--accent)!  transition-opacity duration-200 z-200">
                                  <input
                                    id="buy-btn"
                                    type="number"
                                    className="flex-2 bg-transparent! pr-0  p-4 h-full rounded text-center w-16 overflow-visible "
                                    placeholder="0.0 ETH"
                                    autoFocus
                                    value={bidPrice}
                                  ...
                                  <button
                                    id="buy-btn"
                                    onClick={() => handleBidform()}
                                    className={" h-full bg-transparent! pl-0 p-4 flex-1   text-(--accent)! transition-all duration-200 " + (bidForm ? " w-min" : " w-full")}
                                  >
                                    <span id="buy-btn">Cancel</span>
                                  </button>
                                </div>
                              )}
                              <div id="buy-btn" className="border-b border-(--accent)  w-full"></div>
                              {(tx.txStatus == "idle" || tx.action != "bidToken") && (
                                <button
                                  id="buy-btn"
                                  onClick={() => {
                                    if (bidForm) {
                                      if (Number(bidPrice) > 0) {
                                        handleBidToken(nft.tokenId, Number(bidPrice))
                                      }
                                    } else {
                                      setBidForm(true)
                                    }
                                  }}
                                  className={" h-full bg-transparent! p-4 text-(--accent)! transition-all duration-200 w-full "}
                                >
                                  Bid
                                </button>
                              )}
                              {tx.txStatus == "waiting" && tx.action == "bidToken" && (
                                <button
                                  id="buy-btn"
                                  onClick={() => handleBuy(nft.tokenId, nft.price)}
                                  className=" bg-transparent! w-full p-4  text-(--accent)! hover:opacity-70! transition-opacity duration-200 z-200"
